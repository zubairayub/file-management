...
use Exception;
use App\Mail\PurchaseSubscriptionMail;
    ...
    public function createTransaction(Request $request)
    {
        ...
        if ($isFreePackage || ($response && $response->getMessages()->getResultCode() === 'Ok')) {
            ...
            if ($isFreePackage || ($transactionResponse && $transactionResponse->getResponseCode() == '1')) {
                ...
                // Prepare subscription details with additional parameters
                $subscriptionDetails = [
                ...
                try {
                    Mail::to($user->email)->send(new PurchaseSubscriptionMail($user, $subscriptionDetails));
                    Mail::to($adminEmail)->send(new PurchaseSubscriptionMail($user, $subscriptionDetails));
                } catch (Exception $e) {
